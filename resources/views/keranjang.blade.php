						...
						<?php $no++; ?>
						  <tr>
							...
							<td><img src="<?php if($par->gambarMenu  != "" or $par->gambarMenu  != null){echo  "/images/".$par->gambarMenu;}else{} ?>" alt=" " class="img-fluid" style="width: 300px;height: 200px;"> 
								&nbsp;&nbsp;{{ $par->namaMenu }}</td>
							<td>
							<a href="/front/kurangkeranjang/{{ $par->id }}"><button class="btn btn-danger btn-sm"   >-</button></a>
							&nbsp;&nbsp;
							{{ $par->qty }} &nbsp;&nbsp;
							<a href="/front/tambahkeranjang/{{ $par->id }}"><button class="btn btn-success btn-sm"    >+</button></a>
							</td>
							<td>{{ $par->harga }}</td>
							<td>{{ $par->harga * $par->qty }}</td>
							<td>
							 <a href="/front/hapuskeranjang/{{ $par->id }}"><button class="btn btn-danger btn-sm"   style="margin-left:2px" onclick="return confirm('Yakin akan hapus menu?')" >Hapus</a>
							</td>
